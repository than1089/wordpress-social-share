...
class ThanNgo_Social_Share_Frontend {
	function __construct() {
		add_action( 'init', array( $this, 'init_frontend_social_share') );
	...
	function init_frontend_social_share() {
		...
		if ( isset($options['social_share_positions']['below_title']) ) {
			add_filter( 'the_title', array( $this, 'append_to_title') );
		}
	...
	function append_to_title( $title ) {
		return $title .  $this->get_social_share_html();
	}
	...
	function append_to_thumbnail( $html ) {
		return $html .  $this->get_social_share_html();
	}
	...
	function get_social_share_html( $position = '' ) {
		$options = get_option( 'social_share_settings' );
		$social_networks = array_keys($options['social_share_networks']);
				...
				<?php foreach ($social_networks as $network): ?>
					<li class="share-<?php echo strtolower($network) ?>">
							...
							<?php if ($network == 'Twitter'): ?>
								<a href="https://twitter.com/share?url=<?php echo $post_url; ?>">
							...
							<?php if ($network == 'Pinterest'): ?>
								<a href="https://pinterest.com/pin/create/bookmarklet/?url=<?php echo $post_url; ?>">
							...
							<?php endif ?>
							<?php if ($network == 'Whatsapp' && wp_is_mobile() ): ?>
								<a href="https://api.whatsapp.com/send?text=<?php echo $post_url; ?>">
